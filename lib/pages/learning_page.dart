...
class _LearningPageState extends State<LearningPage> {
  ...
  int _tab = 0;
  final _tabs = ['读书记录', '知识库', '学习成长'];
  final _subTypes = ['book', 'knowledge', 'skill'];
  ...
  void _loadRecords() {
    setState(() => _records = _repo.getRecords('learning', subType: _subTypes[_tab]));
  }
  ...
  void _goToAdd() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AddRecordPage(bookType: 'learning', bookTitle: '学习册'),
      ),
    );
  ...
  void _openSearch() {
    showSearch(context: context, delegate: _LearningSearchDelegate(_repo));
  }
      ...
      appBar: AppBar(
        title: const Row(
          children: [
            ...
            Text('学习册', style: TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
        actions: [
...
class _LearningSearchDelegate extends SearchDelegate {
  final DataRepository _repo;
  _LearningSearchDelegate(this._repo);
  ...
  Widget _buildList() {
    final results = _repo.searchRecords('learning', query);
