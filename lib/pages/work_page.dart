...
class _WorkPageState extends State<WorkPage> {
  ...
  int _tab = 0;
  final _tabs = ['导游事业', '自媒体', 'AI工具'];
  final _subTypes = ['tour_guide', 'media', 'ai_tool'];
  ...
  void _loadRecords() {
    setState(() => _records = _repo.getRecords('work', subType: _subTypes[_tab]));
  }
  ...
  void _goToAdd() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AddRecordPage(bookType: 'work', bookTitle: '工作册'),
      ),
    );
  ...
  void _openSearch() {
    showSearch(context: context, delegate: _WorkSearchDelegate(_repo));
  }
      ...
      appBar: AppBar(
        title: const Row(
          children: [
            ...
            Text('工作册', style: TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
        actions: [
...
class _WorkSearchDelegate extends SearchDelegate {
  final DataRepository _repo;
  _WorkSearchDelegate(this._repo);
  ...
  Widget _buildList() {
    final results = _repo.searchRecords('work', query);
